...
public class edittrip implements Logic
{
	...
	if (trip == null) {
	    context.put("error", "error.no_such_trip");
	    return;
	}

	// make sure this user owns this trip
	if (user.userid != trip.travelerid) {
	    context.put("error", "error.not_owner_of_trip");
	    return;
	}
	...
	if (FormUtil.equals(context, "action", "update")) {
	    ...
	    context.put("error", "message.trip_updated");
	    ...
	    context.put("error", "message.trip_deleted");
